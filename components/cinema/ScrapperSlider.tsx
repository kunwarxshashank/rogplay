...
const TVExpandedItem = ({ item, isFocused, onPress, onFocus, onBlur }: any) => {
    ...
    const imageUrl = item.logo || item.poster_path || item.backdrop_path;
...
let cachedScrapperSliderData: Record<string, any[]> = {};

function ScrapperSlider({ fullScreen = false, variant = 'traditional', fetchFunction, addonType, addonManifestStr, addonUrl, title }: ScrapperSliderProps) {
    const { colors: currentColors } = useTheme();
    const router = useRouter();

    const { data: queryData, isLoading: loading } = useQuery({
        queryKey: ['scrapperslider', addonUrl, title, fetchFunction?.toString()],
        queryFn: () => fetchFunction ? fetchFunction(1) : Promise.resolve([]),
        staleTime: 1000 * 60 * 5,
    });
    ...
    const data = useMemo(() => {
        ...
        if (fullScreen && isTV) {
            ...
            const seen = new Set();
            const filtered = queryData.slice(0, 10).filter((item: any) => {
                const id = item.id?.toString();
                if (!id || seen.has(id)) return false;
                seen.add(id);
                return true;
            });
            return [{ key: 'left-spacer' }, ...filtered, { key: 'right-spacer' }];
        }
    }, [queryData, fullScreen]);
    ...
    const renderItem = useCallback(({ item, index }: { item: any, index: number }) => {
        if (!item.id && !item.backdrop_path && !item.logo) {
        ...
        return (
            ...
            >
                <Animated.View style={[
                    ...
                    <OptimizedImage
                        source={{ uri: item.logo || item.poster_path || item.backdrop_path }}
                        style={styles.poster}
                        transition={0}
                    />
...
export default React.memo(ScrapperSlider);
